...
SDK_OPTIONS = ('debug', 'release', 'environment', 'sample_rate', 'error_sampler', 'max_breadcrumbs', 'attach_stacktrace',
               'send_default_pii', 'event_scrubber', 'include_source_context', 'include_local_variables', 'server_name', 'in_app_include',
               'in_app_exclude', 'max_request_body_size', 'max_value_length', 'ca_certs', 'send_client_reports')
...
class Sentry:
    ...
    def init_app(self, app, dsn=None, debug_enabled=False, spotlight_url=True, detect_spotlight=True,
                 spotlight_cmd="npx @spotlightjs/spotlight", spotlight_script="https://unpkg.com/@spotlightjs/overlay@latest/dist/sentry-spotlight.js",
                 browser=True, browser_options=None, browser_tracing=True, browser_replay=True,
                 browser_script="https://browser.sentry-cdn.com/7.112.2/bundle.tracing.min.js",
                 browser_script_integrity="sha384-Kl6Y90+qOGXFISnB2uX+f/+fgueW0VdsRWpEh9wXsTacad+zDLgOWNjgiqsgYO5i",
                 **sdk_options):
        self.app = app
        ...
        if app.debug and spotlight_url and app.config.get("SPOTLIGHT_DETECT", detect_spotlight):
            try:
                urllib.request.urlopen("http://localhost:8969" if spotlight_url is True else spotlight_url)
        ...
        if app.config.get("SENTRY_BROWSER_TRACING", browser_tracing):
            browser_options.setdefault("integrations", []).append("Sentry.browserTracingIntegration()")
            browser_options.setdefault("tracePropagationTargets", ["localhost", app.config.get("SERVER_NAME") or "localhost:5000"])
            if not app.debug:
        ...
        self.state = state = SentryState(
            ...
            browser_script_integrity=app.config.get("SENTRY_BROWSER_SCRIPT_INTEGRITY", browser_script_integrity),
            enabled=False
        )
        ...
        app.jinja_env.globals.update(
            init_sentry=self.init_frontend,
            init_spotlight=self.init_spotlight
        )

        @app.cli.command()
        def spotlight():
            """Start the Spotlight sidecar"""
            subprocess.run(shlex.split(state.spotlight_cmd) if isinstance(state.spotlight_cmd, str) else state.spotlight_cmd)
    ...
    def init_frontend(self):
        ...
        if self.state.browser:
            if self.state.browser_script:
                script_attrs = {"src": self.state.browser_script, "integrity": self.state.browser_script_integrity, "crossorigin": "anonymous"}
                html = "<script %s></script>\n" % " ".join(f'{k}="{v}"' for k, v in script_attrs.items() if v)
            browser_options = dict(self.state.browser_options, dsn=self.state.sdk_options.get("dsn", "__DSN__"))
